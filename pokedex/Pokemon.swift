...
class Pokemon {
  ...
  func downloadPokemonDetails(completed: DownloadComplete) {
    let url = NSURL(string: _pokemonUrl)! //because we know it's a URL and we know it's going to work
      ...
      if let dict = response.result.value as? Dictionary<String, AnyObject> {
        ...
        if let descArr = dict["descriptions"] as? [Dictionary<String, String>] where descArr.count > 0 {
          ...
          if let url = descArr[0]["resource_uri"] {
            //if there a url and it's a string make another almofire request
            let nsurl = NSURL(string: "\(URL_BASE)\(url)")! // put ! because we know it's going to work
        ...
        } else { // if no description, set to blank
          self._description = ""
        }
